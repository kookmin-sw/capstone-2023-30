...
def create_mesh(depth, image, int_mtx, config):
    ...
    for idx in range(H):
        for idy in range(W):
            ...
            #### save ####
            uvlist.append([y, x])
            depthlist.append([-depth[idx, idy]])
            colorlist.append(image[idx, idy])
    
    import scipy.io as sio
        
    uvlist = np.array(uvlist).astype(np.int16)
    depthlist = np.array(depthlist).astype(np.float32)
    colorlist = np.array(colorlist).astype(np.uint8)
    
    sio.savemat('me.mat', {'uv':uvlist, 'z':depthlist, 'rgb':colorlist, 'h':[H], 'w':[W], 'full_h':[ext_H], 'full_w':[ext_W], 'fov':[60]})
